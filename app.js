...
// sessions = [ { id: string, title: string, messages: [ {role, content} ] } ]
let sessions = JSON.parse(localStorage.getItem('octopus_sessions')) || [];
...
function renderHistoryList() {
  ...
  sessions.forEach(session => {
    ...
    item.innerHTML = `
      ...
      <button class="delete-btn text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition px-1">✕</button>
    `;
...
function renderCurrentSession() {
  ...
  if (!current || current.messages.length === 0) {
    chatBox.innerHTML = `
      <div class="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-slate-300 text-sm mb-4">
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    if (file) {
      if (file.type.startsWith('image/')) {
        alert("Les images ne sont pas encore gérées. Choisis un fichier texte ou code.");
        fileInput.value = '';
        return;
      }
...
async function sendMessage(customPrompt = null) {
  ...
  // Donner un titre basé sur le 1er message
  if (current.messages.length === 0) {
    current.title = text.slice(0, 25) + (text.length > 25 ? '...' : '');
  }
